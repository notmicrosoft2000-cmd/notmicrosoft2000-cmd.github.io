(function () {
  ...
  var AI_NOTE = "This description was generated by an AI system. It may not accurately represent the project. Check the repository for authoritative information.";
  ...
  var NEWS = [
    { cat: "TOP", time: "just now", title: "NEPT stock holds above $47 despite irregular annual filing", body: "Analysts remain uncertain about a company whose latest 10-K filing appears to be a sandwich. Management has not commented. Shares are up marginally." },
    { cat: "MARKETS", time: "3 min ago", title: "Trading volume surges past 4 million on misplaced financial blog listing", body: "Unprecedented interest in NEPT shares attributed to a blog that listed Neptune Productions under companies that definitely exist. The exchange has declined to comment." },
    { cat: "SCANDAL", time: "8 min ago", title: "Board meeting agenda reportedly consumed before session could begin", body: "An emergency board meeting was called to address the sandwich-shaped filing. The agenda was lost before the meeting started. Minutes were sparse." },
    { cat: "TECH", time: "14 min ago", title: "NeptuneOS orb display continues uninterrupted since 2019", body: "The NeptuneOS startup orb, active since 2019, shows no sign of stopping. Engineers have stopped trying to power it down. A formal report was filed." },
    { cat: "LOCAL", time: "22 min ago", title: "Delivery delays continue across Neptune Productions courier network", body: "Package delivery times remain at 3\u20137 business days following the restructuring of the company's courier operations. Management says improvements are planned." },
    { cat: "TRAGEDY", time: "31 min ago", title: "Legacy system memory limit declared sufficient for 847th consecutive time", body: "NeptuneDOS has reiterated that its 640K base memory is adequate for all operations. Independent review has been requested but not yet scheduled." }
  ];

  var FILINGS = [
    ["10-K", "Annual report. Filed as-is."],
    ["10-Q", "Quarterly report. Pending review."],
    ["8-K", "Current report. Board changes noted."],
    ["DEF 14A", "Proxy statement. Voting instructions included."],
    ["S-1", "Registration statement. Under review."]
  ];

  var BOARD = [
    ["Neptune Holdings LLC", "Majority shareholder"],
    ["Independent Director", "Audit committee chair"],
    ["Independent Director", "Compensation committee"],
    ["Management Director", "CEO"],
    ["Management Director", "CFO"],
    ["Advisory Board", "Non-voting observer"]
  ];
  ...
  var REVENUE = [
    ["Software Licenses", "$2.8M", "32%"],
    ["SaaS Subscriptions", "$1.9M", "22%"],
    ["Support & Services", "$1.4M", "16%"],
    ["Hardware Peripherals", "$1.1M", "13%"],
    ["Consulting", "$680K", "8%"],
    ["Licensing Fees", "$340K", "4%"],
    ["Other", "$180K", "2%"],
    ["Returns & Adjustments", "-$420K", "-5%"],
    ["NET REVENUE", "$8.0M (est.)", "100%"]
  ];

  var EARNINGS = [
    ["Q4 2025", "$8.0M", "$0.14", "In line with expectations"],
    ["Q3 2025", "$7.6M", "$0.12", "Slight beat on revenue"],
    ["Q2 2025", "$7.2M", "$0.11", "Below guidance"],
    ["Q1 2025", "$8.4M", "$0.16", "Strong quarter"],
    ["Q4 2024", "$7.1M", "$0.10", "Seasonal softness"],
    ["Q3 2024", "$7.8M", "$0.13", "Steady growth"]
  ];

  var SHAREHOLDERS = [
    ["Neptune Holdings LLC", "34.2%", "4,378K"],
    ["Vanguard Group", "12.8%", "1,638K"],
    ["CEO (restricted)", "11.5%", "1,472K"],
    ["BlackRock Inc.", "8.9%", "1,139K"],
    ["FMR LLC", "6.3%", "806K"],
    ["State Street Corp.", "5.1%", "653K"],
    ["Insiders (other)", "4.8%", "614K"],
    ["Public Float", "16.4%", "2,100K"]
  ];

  var CONTACTS = [
    { label: "EMAIL", value: "productions@neptune.test", note: "Checked hourly, answered occasionally.", href: "mailto:productions@neptune.test" },
    { label: "GITHUB", value: "notmicrosoft2000-cmd", note: "All source code, all repositories.", href: "https://github.com/notmicrosoft2000-cmd" },
    { label: "DISCORD", value: "the hallway", note: "Community server. Invite posted in the channel." },
    { label: "RADIO", value: "87.7 FM", note: "Low-power broadcast. Reception varies." },
    { label: "INVESTOR RELATIONS", value: "ir@neptune.test", note: "Filings and correspondence. Response time: variable." },
    { label: "TRANSFER AGENT", value: "Neptune Registrar", note: "Holdings and transfer inquiries. 3\u20137 business days." }
  ];

  var HOME_NEWS = [
    ["LIVE", "NEPT trading active. Market data updated every 5 seconds."],
    ["08.16.2026", "Navigation lives on the left edge. Hover or tap to switch between programs."],
    ["08.16.2026", "Quick settings accessible via the logo, top-left corner."],
    ["08.16.2026", "New admin control panel available for authorized operators."],
    ["08.16.2026", "Contact the Productions by email, GitHub, or Discord."],
    ["08.16.2026", "All project websites accessible from the home grid below."]
  ];
  ...
  var Market = (function () {
    ...
    function hashDate(s) { var h = 2166136261; for (var i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 16777619); } return h >>> 0; }
    function build() {
      ...
      for (var d = 251; d >= 0; d--) {
        ...
        var dk = keyOf(dd);
        var dr = mulberry32(hashStr(dk));
        dp = Math.max(5, dp * (1 + (dr() - 0.48 + bias * 0.5) * (vol * 1.8)));
        var dv = Math.round(800 + dr() * 5200);
        histDaily.push({ p: dp, v: dv, label: (dd.getMonth() + 1) + "/" + dd.getDate() });
      }
      if (series.length) {
        histDaily.push({ p: series[series.length - 1], v: volume, label: "today" });
      }
    }
    ...
    function getRange(range) {
      ...
      if (range === "1D") return { prices: series.slice(0, idx() + 1), volumes: volBar.slice(0, idx() + 1), labels: (function () { var l = []; for (var i = 0; i <= idx(); i++) { var m = Math.floor(i * 6 / 60); var h = 9 + Math.floor((i * 6 + 570) / 60); var mm = (i * 6 + 570) % 60; if (mm === 0 || i === 0) l.push({ i: i, t: (h > 12 ? h - 12 : h) + ":" + (mm < 10 ? "0" : "") + mm }); } return l; })() };
      var counts = { "5D": 5, "1M": 22, "3M": 66, "1Y": 252, "ALL": 252 };
      var n = counts[range] || 252;
      var slice = histDaily.slice(Math.max(0, histDaily.length - n));
      return { prices: slice.map(function (d) { return d.p; }), volumes: slice.map(function (d) { return d.v; }), labels: (function () { var l = []; var step = Math.max(1, Math.floor(slice.length / 6)); for (var i = 0; i < slice.length; i += step) { l.push({ i: i, t: slice[i].label }); } return l; })() };
    }
    return {
      ensure: ensure,
      build: build,
      getRange: getRange,
      price: function () { ensure(); return series[idx()] || 0; },
      change: function () { ensure(); return ((series[idx()] || 0) / open - 1) * 100; },
      ...
      mcap: function () { ensure(); return ((series[idx()] || 0) * (12800 + hashStr(dayKey) % 3200) / 1000).toFixed(0); },
      pe: function () { ensure(); return ((series[idx()] || 0) / (open * 0.04 + 0.12)).toFixed(1); },
  ...
  function renderMarket() {
    ...
    if (brk) {
      ...
      if (brkOver) { brk.textContent = brkOver; }
        ...
        var bv = ch >= 0
          ? "BREAKING \u2014 NEPT holds above $" + Market.fmt(p) + ". trading volume normal. analysts watching closely."
          : "BREAKING \u2014 NEPT down " + Math.abs(ch).toFixed(1) + "% on light volume. no official comment from management.";
        brk.textContent = bv;
      }
    }
  }
  ...
  function renderChart(range) {
    var svg = $("#nepChart");
    if (!svg) return;
    var data = Market.getRange(range || "1D");
    var all = data.prices;
    var allV = data.volumes;
    var tLabels = data.labels;
    if (all.length < 2) return;
    var W = 800, H = 300, PAD = { t: 20, r: 12, b: 28, l: 56 }, VH = 60;
    var cw = W - PAD.l - PAD.r, ch = H - PAD.t - PAD.b - VH - 6;
    ...
    mn -= pad; mx += pad;
    for (var i = 0; i < allV.length; i++) { if (allV[i] > mxV) mxV = allV[i]; }
    var isUp = all[all.length - 1] >= all[0];
    var lineCol = isUp ? "#43d97a" : "#ff5c5c";
    var s = "";
    s += '<defs><linearGradient id="chartGrad" x1="0" y1="0" x2="0" y2="1">' +
      '<stop offset="0%" stop-color="' + lineCol + '" stop-opacity=".22"/>' +
      '<stop offset="100%" stop-color="' + lineCol + '" stop-opacity=".01"/>' +
      '</linearGradient></defs>';
    ...
    for (var j = 0; j < tLabels.length; j++) {
      var tx = PAD.l + (tLabels[j].i / Math.max(1, all.length - 1)) * cw;
      if (tx <= PAD.l + cw + 5) {
        s += '<line x1="' + tx + '" y1="' + PAD.t + '" x2="' + tx + '" y2="' + (PAD.t + ch) + '" stroke="var(--line)" stroke-width=".5" stroke-dasharray="3,3"/>';
        s += '<text x="' + tx + '" y="' + (PAD.t + ch + 14) + '" text-anchor="middle" fill="var(--muted)" font-size="9" font-family="var(--font-m)">' + tLabels[j].t + '</text>';
      }
    }
    var linePath = "", areaPath = "";
    for (var i = 0; i < all.length; i++) {
      var x = PAD.l + (i / (all.length - 1)) * cw;
      var yy = PAD.t + (1 - (all[i] - mn) / (mx - mn)) * ch;
      linePath += (i === 0 ? "M" : "L") + x.toFixed(1) + "," + yy.toFixed(1);
      areaPath += (i === 0 ? "M" : "L") + x.toFixed(1) + "," + yy.toFixed(1);
    }
    var axEnd = PAD.l + ((all.length - 1) / (all.length - 1)) * cw;
    areaPath += "L" + axEnd.toFixed(1) + "," + (PAD.t + ch) + "L" + PAD.l + "," + (PAD.t + ch) + "Z";
    s += '<path d="' + areaPath + '" fill="url(#chartGrad)"/>';
    s += '<path d="' + linePath + '" fill="none" stroke="' + lineCol + '" stroke-width="1.8" stroke-linejoin="round" stroke-linecap="round"/>';
    var lx = PAD.l + ((all.length - 1) / (all.length - 1)) * cw;
    var ly = PAD.t + (1 - (all[all.length - 1] - mn) / (mx - mn)) * ch;
    s += '<circle cx="' + lx.toFixed(1) + '" cy="' + ly.toFixed(1) + '" r="4" fill="' + lineCol + '"/>';
    s += '<circle cx="' + lx.toFixed(1) + '" cy="' + ly.toFixed(1) + '" r="8" fill="' + lineCol + '" opacity=".18"/>';
    s += '<line x1="' + lx.toFixed(1) + '" y1="' + ly.toFixed(1) + '" x2="' + lx.toFixed(1) + '" y2="' + (PAD.t + ch) + '" stroke="' + lineCol + '" stroke-width=".7" stroke-dasharray="4,3"/>';
    s += '<line x1="' + PAD.l + '" y1="' + ly.toFixed(1) + '" x2="' + lx.toFixed(1) + '" y2="' + ly.toFixed(1) + '" stroke="' + lineCol + '" stroke-width=".7" stroke-dasharray="4,3"/>';
    var vTop = PAD.t + ch + 6;
    var barW = Math.max(1.5, Math.min(3, cw / all.length * 0.7));
    for (var i = 0; i < allV.length; i++) {
      var bx = PAD.l + (i / (all.length - 1)) * cw;
      var bh = mxV > 0 ? (allV[i] / mxV) * VH : 0;
      var bcol = (i > 0 && all[i] >= all[i - 1]) ? "#43d97a" : "#ff5c5c";
      s += '<rect x="' + (bx - barW / 2).toFixed(1) + '" y="' + (vTop + VH - bh).toFixed(1) + '" width="' + barW.toFixed(1) + '" height="' + bh.toFixed(1) + '" fill="' + bcol + '" opacity=".45"/>';
    }
    svg.innerHTML = s;
  }
  ...
  function buildViews() {
    ...
    html += '<section class="view" data-view="home" role="tabpanel">' +
      ...
      '<div class="home-foot">' +
        ...
        '<span class="home-foot-text">Market data is simulated and does not represent actual securities. Not investment advice.</span>' +
      '</div>' +
    '</section>';
  ...
  function buildNewsView() {
    ...
    var tickerText = allNews.map(function (n) { return n.title; }).join("  \u2022  ") + "  \u2022  ";
    ...
    var html = '<section class="view" data-view="news" role="tabpanel">' +
      '<div class="news-masthead">' +
        ...
        '<span class="news-tag">NEPTUNE PRODUCTIONS \u2014 MARKET &amp; CORPORATE DESK. SIMULATED COVERAGE FOR DEMONSTRATION PURPOSES.</span>' +
      '</div>' +
      ...
      '<div class="news-feature">' +
        '<div class="nf-body">' +
          ...
          '<h3>' + feat.title + '</h3>' +
          '<p>' + feat.body + '</p>' +
          '<span class="nmeta"><b>NEPT News Desk</b> \u2022 ' + feat.time + '</span>' +
        '</div>' +
      '</div>' +
      '<div class="news-grid">' + rest.map(function (n) {
        return '<article class="ncard" data-cat="' + n.cat + '"><span class="ncat">' + n.cat + '</span>' +
          '<h3>' + n.title + '</h3>' +
          '<p>' + n.body + '</p>' +
          '<span class="nmeta"><b>NEPT News Desk</b> \u2022 ' + n.time + '</span></article>';
      }).join("") + '</div>' +
      '<p class="news-note">This is simulated coverage for demonstration purposes. Market data is generated algorithmically. Not investment advice.</p>' +
    '</section>';
    return html;
  }
  ...
  function buildInvestView() {
    var html = '<section class="view" data-view="investors" role="tabpanel">' +
      '<div class="inv-head">' +
        ...
        '<p>NEPTUNE PRODUCTIONS INC. \u2014 NASDAQ LISTED. SYMBOL: NEPT.</p>' +
      '</div>' +
      '<div class="inv-hero">' +
      ...
      '<div class="inv-about">' +
        '<h3>About</h3>' +
        '<p>Neptune Productions Inc. is a diversified technology conglomerate engaged in software development, digital media, and operating systems. Founded in 1987, the company operates across multiple segments including application software, system utilities, and network tools. Neptune Productions is headquartered with operations spanning multiple platforms. The company\'s common stock trades on the NASDAQ under the ticker NEPT.</p>' +
      '</div>' +
      '<div class="inv-breakdown">' +
        '<div class="inv-half">' +
          win("nep", "revenue breakdown (quarterly, unaudited)", '<table class="inv-table">' +
            ...
            '</tbody></table>' +
            '<p class="inv-footnote">Segment data is unaudited. Figures in USD.') +
          win("nep", "earnings history", '<table class="inv-table">' +
            ...
            '</tbody></table>' +
            '<p class="inv-footnote">All figures in USD. Previous quarter restated following filing corrections.') +
        '</div>' +
        '<div class="inv-half">' +
          win("nep", "shareholder structure", '<table class="inv-table">' +
            ...
            '</tbody></table>' +
            '<p class="inv-footnote">Share counts estimated. Data as of latest filing.') +
        ...
        '</div>' +
      '</div>' +
      '<p class="news-note">This page is for informational purposes only. Market data is simulated. Not investment advice.</p>' +
    '</section>';
    return html;
  }
  ...
  try {
    fetch("data.json?t=" + Date.now()).then(function (r) { return r.ok ? r.json() : null; }).then(function (d) {
      ...
      if (d.stock && Object.keys(d.stock).length) {
        try { localStorage.setItem("hub_adm_stock", JSON.stringify(d.stock)); } catch(e) {}
      }
      if (d.breaking) {
        try { localStorage.setItem("hub_adm_breaking", d.breaking); } catch(e) {}
      }
      if (d.ticker) {
        try { localStorage.setItem("hub_adm_ticker", d.ticker); } catch(e) {}
      }
      if (d.news && d.news.length) {
        try { localStorage.setItem("hub_adm_news", JSON.stringify(d.news)); } catch(e) {}
      }
      renderMarket();
    }).catch(function () {});
  } catch(e) {}
